    ...
    def next_move(self):
        if - math.pi / 2 < self.kicker.ball.angle < math.pi / 2:
            if self.kicker.ball.pos[Coordinate.Y] < COURT_HEIGHT / 2:
               ...
               # new_pos_defender = self.kicker.ball.pos[Coordinate.Y] - self.kicker.human_defender.POSITION_ON_BAR_DEFENDER_LEFT + FIGURE_FOOT_HEIGHT / 2

            elif self.kicker.ball.pos[Coordinate.Y] > COURT_HEIGHT / 2:
                new_pos_keeper = self.kicker.ball.pos[Coordinate.Y] - self.kicker.human_keeper.POSITION_ON_BAR \
                                 + FIGURE_FOOT_HEIGHT / 2
               # new_pos_defender = self.kicker.ball.pos[Coordinate.Y] - self.kicker.human_defender.POSITION_ON_BAR_DEFENDER_RIGHT - FIGURE_FOOT_HEIGHT / 2
            else:
                new_pos_keeper = self.kicker.ball.pos[Coordinate.Y] - self.kicker.human_keeper.POSITION_ON_BAR
               # new_pos_defender = self.kicker.ball.pos[Coordinate.Y] - self.kicker.human_defender.POSITION_ON_BAR_DEFENDER_RIGHT - FIGURE_FOOT_HEIGHT / 2
